...
def main():
    ...
    save(OUT/'primary_comparisons.json',pairs);files.append(OUT/'primary_comparisons.json');assert len(pairs)==60
    finish(run,files,primary_contrasts=60,histories=13,methods=6,seed_fusion_rules_changed=False,calibration_fits=0,historical_extension=True,independent_holdout=False)
    m=tables['ensemble_metrics'];print(m[m.method.isin(PRIMARY)&m.history.isin(list(ARMS.values()))&m.window.ne('pooled_2021_2026')][['window','history','method','correct_directions','n','accuracy','brier','log_loss']].to_string(index=False),flush=True)
