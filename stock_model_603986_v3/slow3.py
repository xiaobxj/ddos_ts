...
def raw_banks():
    ...
    for cadence in ['annual','quarterly']:
        for c in all_cutoffs(cadence):
            ...
            for name,p in [('frequency',np.full(len(ids),done['training_frequency'])),('market4',market),('constant50',np.full(len(ids),.5))]:
...
def calibrated_banks(raw):
    ...
    for (cadence,method),bank in raw.groupby(['cadence','method'],sort=False):
        ...
        if method=='vol':
            ...
            prior=raw[raw.cadence.eq(cadence)&raw.method.eq('frequency')].set_index('date').probability
            b['probability']=b.date.map(prior)
            for j,c in enumerate(ends):
...
def main():
    ...
    amendment=read(OUT/'evaluation_freeze_amendment.json')
    assert amendment['previous_freeze_sha256']==sha(OUT/'evaluation_freeze.json')
    for p,s in amendment['files'].items():assert sha(ROOT/p)==s,p
