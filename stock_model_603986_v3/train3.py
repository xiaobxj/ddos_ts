"""27 new H5 quarterly fits, using the unchanged v2 numerical training recipe."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--lane',type=int,choices=[0,1],required=True);a=ap.parse_args()
    check_freeze();modules(True)
    chosen=[t for i,t in enumerate(tasks()) if i%2==a.lane]
    for task in chosen:fit(task)
    check_freeze();save(OUT/f'training_lane{a.lane}_completed.json',dict(status='PASS',completed_utc=now(),neural_fits=3*len(chosen),tasks={task_id(t):sha(OUT/'fits'/task_id(t)/'completed.json') for t in chosen}))
