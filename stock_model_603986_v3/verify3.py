...
def outputs():
    ...
    assert p[p.method.eq('constant50')].probability.eq(.5).all()
    for cadence in ['annual','quarterly']:
        a=p[p.cadence.eq(cadence)&p.method.eq('cal_frequency')&p.date.str.slice(5,7).isin(['01','02','03'])].sort_values('date')
        b=p[p.cadence.eq(cadence)&p.method.eq('frequency')&p.date.isin(a.date)].sort_values('date')
