"""Write a factual report; no model or policy selection occurs here."""
...
def main():
    s=load_csv(OUT/'slow_metrics.csv').set_index('name');y=load_csv(OUT/'slow_years.csv');fast=load_csv(OUT/'fast_metrics.csv').set_index('name')
    audit=read(OUT/'final_audit.json');assert audit['status']=='PASS';v=read(OUT/'models_verified.json');o=read(OUT/'outputs_verified.json')
    events=d.chain(d.LIVE);first=next(e for e in events if e['kind']=='forecast');forecast=first['payload']
    selected=[('annual.vol.raw','年度更新原始'),('quarterly.vol.raw','季度更新原始'),('quarterly.vol.platt','季度更新＋现有校准'),
              ('quarterly.vol.shrink','季度更新＋受约束收缩（事先主候选）'),('quarterly.frequency.raw','同期训练上涨频率'),
              ('quarterly.cal_frequency.raw','校准训练窗口上涨频率'),('quarterly.market4.raw','同期 4 项市场特征模型'),('quarterly.constant50.raw','恒定 50%')]
    lines=['# 603986：快慢周期第三轮测试与每日程序','',
        ...
        table(['方法','方向正确率','Brier','对数损失'],[[label,f'{s.loc[name,"accuracy"]:.2%}',f'{s.loc[name,"brier"]:.6f}',f'{s.loc[name,"logloss"]:.6f}'] for name,label in selected]),'',
        ...
        '',
        table(['年份','样本数','年度原始正确率','季度原始正确率','季度收缩正确率','季度原始 Brier','季度收缩 Brier'],
              [[str(year),str(int(y[(y.name=='annual.vol.raw')&(y.year==year)].iloc[0]['n']))]+[
                  f'{y[(y.name==name)&(y.year==year)].iloc[0][field]:.2%}' if field=='accuracy' else f'{y[(y.name==name)&(y.year==year)].iloc[0][field]:.6f}'
                  for name,field in [('annual.vol.raw','accuracy'),('quarterly.vol.raw','accuracy'),('quarterly.vol.shrink','accuracy'),('quarterly.vol.raw','brier'),('quarterly.vol.shrink','brier')]] for year in [2023,2024,2025,2026]]),'',
        ...
        '',
        table(['特征','方向正确率','Brier'],[[label,f'{fast.loc[name,"accuracy"]:.2%}',f'{fast.loc[name,"brier"]:.6f}'] for name,label in [('frequency','训练上涨频率'),('own','仅个股短期特征'),('market','增加沪深300'),('sector','增加国证芯片'),('both','同时增加二者')]]),
        '',
        ...
        '',
        '## 每日程序已投入前瞻记录','',
        f'第一笔真实预测于北京时间 **{datetime.datetime.fromisoformat(first["recorded_utc"]).astimezone(d.CST):%Y-%m-%d %H:%M:%S}** 保存，信号日收盘价 {forecast["raw_close"]:.2f} 元。现在只有 1 个真实信号日、0 个已成熟标签；历史回测没有导入前瞻正确率。',
        '',
        table(['预测','输出','目标收盘日'],[[d.NAMES[row['name']],f'{row["probability"]:.2%}',forecast['targets'][str(row['h'])]] for row in forecast['rows']]),
        '',
        ...
        '',
        '## 验证与保留记录','',
        f'- {v["checkpoints"]} 个新增检查点、{v["heads"]} 个本轮使用的概率头重算通过；预测最大差 {v["max_p_delta"]:.3g}（GPU 单批推理浮点差异），优化梯度最大 {v["max_gradient"]:.3g}。9 个新增市场基准模型另做系数与预测复核。',
        f'- {o["calibration_prefix_checks"]} 次校准的未来标签剔除／截断复核通过；旧年度主模型和旧校准概率在 2e-15 容差内完整重现。',
        ...
        f'- 原有 {o["old_files_preserved"]} 个文件逐一核对哈希，指数程序、旧前瞻账本及个股 v1/v2 未修改。本轮训练／评分与前瞻记录分目录保存。',
    ...
    (ROOT/'README.md').write_text('''# 603986 快慢周期第三轮
