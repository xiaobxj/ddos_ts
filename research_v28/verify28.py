...
from contract28 import memberships,verify_routes
import evaluate28 as evaluation
...
def main():
    ...
    obs,price,targets=data();models=read(OUT/'models.json');heads=read(OUT/'heads.json');testrefs=read(OUT/'testing_features.json');pred=csv('unique_model_predictions');ensemble=csv('ensemble_predictions');curves=csv('training_curves');scales=read(OUT/'training_scales.json');solutions=[];replays=[];maxpred=0.;maxfeatures=0.
    for r in models:
        ...
        for row in cs.itertuples():
            assert row.order_sha256==training.array_hash(schedule(len(tr),len(tr),rng)) and row.presentations==len(tr) and row.optimizer_steps==(len(tr)+127)//128
        ...
        assert training.object_hash(model.state_dict())==r['model_sha256'];del model,values;torch.cuda.empty_cache();print(f'Verified {len(replays)}/138 backbones and {len(solutions)}/552 solvers',flush=True)
    unique=pred;pred=csv('model_predictions');base,base_e=frozen_baselines();pd.testing.assert_frame_equal(pred[~pred.history.isin(NEW)].reset_index(drop=True),base,check_dtype=False,atol=1e-14,rtol=0)
    ...
    assert len(pred)==30464 and len(ensemble)==11424
    assert len(solutions)==552 and len(replays)==138;assert old_evidence()==prep['old_evidence'];check_frozen()
    ...
    save(OUT/'verification.json',dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],old_files_preserved=len(prep['old_evidence']),membership_checks=len(dcheck),checkpoint_replays=138,independent_head_solutions=552,maximum_feature_algebra_gap=maxfeatures,maximum_forecast_gap=maxpred,training_feature_rows=sum(r['train_n'] for r in models),testing_feature_rows=sum(f['test_n'] for f in cfg()['folds'])*6,independent_metrics=True,all_predefined_comparisons_recomputed=True,artifacts={n:sha(OUT/n) for n in ['independent_solutions.csv','checkpoint_replays.csv']}))
