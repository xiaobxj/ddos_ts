...
def synthetic():
    ...
    z=np.zeros(52);y=np.r_[np.ones(40),np.zeros(12)];g=pd.DataFrame(dict(annual_logit=z,actual_up=y,state=[STATES[0]]*10+[STATES[1]]*9+[STATES[2]]*33));h=fitted(g,'ready');assert h[0]['fit_eligible'] and not h[1]['fit_eligible'] and h[1]['offset']==0 and h[4]['fit_eligible'];assert all(x['offset']==0 for x in fitted(g,'annual_reset'))
    ...
    return ['purge_unmatured_adjacent_training_label','strict_training_before_validation','52plus13count_requirement','annual_reset','state_train10boundary','state_validation5boundary','Brier_tie_rejected','Brier_gain_cannot_override_worse_direction','zero_offset_exact_copy']
def build():
    ...
    return bank,pd.DataFrame(schedules),pd.DataFrame(members),pd.DataFrame(support)
def main():
    ...
    check_frozen(False);save(OUT/'contract_verification.json',dict(status='PASS',started_utc=start,completed_utc=now(),protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],synthetic_cases=cases,ready_quarters=int(tables[1]['mode'].eq('ready').sum()),first_ready= tables[1][tables[1]['mode'].eq('ready')].cutoff.min(),artifacts={str(p.relative_to(ROOT)):sha(p) for p in files}));print('Contract PASS: purged52/13 splits, archived point-in-time signals and support frozen.',flush=True)
