...
from contract39 import synthetic
from validate39 import validate
...
def check_splits():
    ...
    for c in csv('schedule').itertuples():
        available=sorted([r for r in pool.itertuples() if isinstance(r.joint_completed,str) and r.joint_completed<=c.cutoff],key=lambda r:r.date);val=available[-13:];fit_cutoff=(pd.Timestamp(val[0].date)-pd.Timedelta(days=1)).strftime('%Y-%m-%d') if val else None;train=sorted([r for r in pool.itertuples() if fit_cutoff and r.joint_completed<=fit_cutoff],key=lambda r:r.date)[-52:];mode='annual_reset' if c.cutoff.endswith('12-31') else 'ready' if len(train)==52 and len(val)==13 else 'insufficient_history';assert c.mode==mode and c.available==len(available) and c.train_n==len(train) and c.validation_n==len(val);eq(c.fit_cutoff,fit_cutoff);nready+=mode=='ready'
    ...
    for c in schedule.itertuples():
        ...
        for m in LEARNED:
            for seed in cfg()['seeds']:
                tr=input_rows(bank,trids,m,seed);hs=[hi[(c.cutoff,m,seed,family,comp)] for family,comps in [('state',STATES),('global',['all'])] for comp in comps]
                if c.mode=='ready':
...
def check_predictions():
    ...
    for r in routing.itertuples():
        ...
        assert r.offset==off and r.validation_accepted==g.accepted and r.fit_eligible==h['fit_eligible'] and r.gate_reason==g.reason and r.state==source.state and r.cutoff==source.head_cutoff<r.date;assert actual.direction_up==int(actual.probability>.5) and actual.actual_up==source.actual_up and r.probability==actual.probability
        if off==0:assert actual.probability==source.probability;fallback+=1
    assert len(routing)==13056
    for history in NEW:
    ...
    for r in csv('quarter_outcomes').itertuples():
...
def check_inference():
    ...
    np.testing.assert_array_equal(adjusted,[r['holm_adjusted_p'] for r in pairs]);assert len(pairs)==84
def main():
    ...
    synthetic();ready=check_splits();sol,gates,fit=check_fits_and_gates();print('Independent purged splits, solvers, validation gates and information isolation PASS.',flush=True);pred=check_predictions();print('Independent forecast routing, fallback, metrics and quarter diagnostics PASS.',flush=True);check_inference();assert old_evidence()==prep['old_evidence'];assert fit['independent_scalar_solutions']==read(OUT/'fitting_manifest.json')['scalar_fits'];files=[]
    ...
    save(OUT/'verification.json',dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],ready_quarters=ready,old_files_preserved=5484,independent_comparisons=84,**fit,**pred,artifacts={str(p.relative_to(ROOT)):sha(p) for p in files}));print('R39 independent verification PASS.',flush=True)
