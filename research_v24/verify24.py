...
def main():
    ...
    archive=PROJECT/'research_v24_attempt1';old_protocol=read(archive/'protocol.json')
    for key,value in old_protocol.items():
        if key!='budget':assert cfg()[key]==value,key
    for filename,frame in [('new_model_predictions',new),('model_predictions',predictions),('ensemble_predictions',ensemble)]:
        failed=pd.read_csv(archive/'results'/f'{filename}.csv',float_precision='round_trip');pd.testing.assert_frame_equal(failed,frame,check_dtype=False,rtol=0,atol=0)
    archived_heads=read(archive/'results/heads.json')
    for a,b in zip(archived_heads,heads):assert a==b
    ...
    result=dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,previous_files_preserved=3475,previous_round_files_preserved=3389,failed_attempt_files_preserved=86,archived_primary_fits=48,total_round_primary_fit_executions=96,attempt_forecasts_identical=True,new_primary_fits=48,independent_solutions=48,new_projection_fits=0,reused_projection_fits=24,inherited_feature_split_checks=48,temporal_provenance_chains=24,
