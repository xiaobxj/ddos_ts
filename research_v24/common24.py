...
def old_evidence():
    ...
    archive=PROJECT/'research_v24_attempt1'
    for name,digest in read(archive/'attempt_preservation.json')['files'].items():r[str((archive/name).relative_to(PROJECT))]=digest
    path=archive/'attempt_preservation.json';r[str(path.relative_to(PROJECT))]=sha(path);assert len(r)==3475
