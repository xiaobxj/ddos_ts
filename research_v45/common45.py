...
def old_evidence():
    ...
    attempt=PROJECT/'research_v45_attempt1';a=read(attempt/'results/attempt_manifest.json');r.update({str((attempt/n).relative_to(PROJECT)):h for n,h in a['files'].items()});p=attempt/'results/attempt_manifest.json';r[str(p.relative_to(PROJECT))]=sha(p);assert len(r)==5916
