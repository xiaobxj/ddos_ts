...
def main():
    ...
    '', '此前所有历史均已查看，本轮没有新增盲测；多重比较校正不覆盖此前自适应研究。分组变粗可能增加支持，也可能丢失市场状态差异；不得仅按覆盖或单个年份选择候选。没有部署或交易收益测试。5,916 个旧文件、原预测历史及神经模型保持不变。',
    ...
    lines+=['','数值实现修复：首次运行使用 GroupBy 聚合均值，与原逐组 Series.mean 的浮点归约顺序不同，使 284 条零偏移集成预测出现不超过 2.22×10⁻¹⁶ 的差异，被误计为概率变化。该次运行完整保留在 research_v45_attempt1，不能作为覆盖率结果使用。本报告使用恢复原集成算法后的完整重跑结果，并新增集成回退精确相同检查。参数、门控、验证种子预测和全部模型种子预测文件与首次运行逐字节相同，没有调整科学方案。5,916 个旧文件含原 5,848 个历史文件与首次运行的 68 个留存文件。']
