...
def main():
    ...
    repair_names=['correction_heads.json','correction_parameters.csv','gate_decisions.csv','validation_predictions.csv','model_predictions.csv','seed_routing.csv']
    for n in repair_names:assert sha(OUT/n)==sha(PROJECT/'research_v45_attempt1/results'/n),n
    save(OUT/'verification.json',dict(status='PASS',started_utc=start,completed_utc=now(),protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],old_files_preserved=5916,old_histories_preserved=23,new_neural_fits=0,independent_comparisons=48,numerical_repair_unchanged_fit_gate_seed_files=repair_names,invalid_attempt_files_preserved=68,**fit,**pred,**diag,artifacts={str(p.relative_to(ROOT)):sha(p) for p in files}));print('R45 independent predictions, metrics, diagnostics and48comparisons PASS.',flush=True)
