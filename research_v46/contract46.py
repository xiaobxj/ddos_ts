...
def main():
    ...
    tests.append('child_support_exactly_matches_original_four_state')
    check_frozen(False);save(OUT/'contract_verification.json',dict(status='PASS',completed_utc=now(),protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],synthetic_cases=tests,ready_quarters=13,cutoffs=23,group_support_cells=184,artifacts={str(p.relative_to(ROOT)):sha(p)}));print('Contract PASS: original52/13maturity splits, strict routing and shared parent fits with original child-state support; frozen.',flush=True)
