...
def check_fits_and_validation():
    ...
    assert len(heads)==1104 and len(hi)==1104 and len(gates)==736
    ...
    for r in gates.itertuples():
        assert independent_group(r.state,r.family)==r.component
        ids=[i for i in rows.get((r.cutoff,'validation'),[]) if state[i]==r.state];bases=[];probs=[];labels=[]
        for idx in ids:
            bp=[];pq=[]
        ...
        trainids=[i for i in rows.get((r.cutoff,'training'),[]) if state[i]==r.state]
    ...
    return dict(independent_scalar_solutions=fitcount,maximum_coefficient_gap=maxgap,maximum_kkt_violation=maxkkt,independent_gate_cells=736,independent_validation_seed_rows=len(vp),outside_training_poison_cutoffs=fitpoison,postcutoff_validation_poison_cutoffs=valpoison),[p]
def check_predictions_metrics():
    models=csv('model_predictions');ensemble=csv('ensemble_predictions');base=csv('baseline_model_predictions');be=csv('baseline_ensemble_predictions');details=csv('seed_routing');bank=csv('weekly_signal_bank');heads=read(OUT/'correction_heads.json');gates=csv('gate_decisions').set_index(['cutoff','method','family','state']);hi={(h['cutoff'],h['method'],h['seed'],h['family'],h['component']):h for h in heads};bi=bank.set_index(['method','seed','row_index']);mi=models.set_index(['history','method','seed','row_index']);ei=ensemble.set_index(['history','method','row_index']);route=csv('routing').set_index('row_index')
    ...
    assert len(models)==117504 and len(ensemble)==44064 and len(details)==6528 and not details[['history','method','seed','row_index']].duplicated().any();maxgap=0.;zeros=0
    for r in details.itertuples():
        ...
        h=hi[(r.cutoff,r.method,r.seed,r.family,group)];gate0=gates.loc[(r.cutoff,r.method,r.family,b.state)];off=h['offset'] if gate0.accepted else 0.;a=mi.loc[(r.history,r.method,r.seed,r.row_index)]
    ...
    exact_ensemble_fallback=0
    offset_groups=details.groupby(['history','method','row_index']).offset.apply(lambda x:bool(x.eq(0).all())).to_dict()
    for (history,method,idx),g in models.groupby(['history','method','row_index'],sort=False):
        ...
        if history in NEW and method in METHODS and offset_groups[(history,method,idx)]:
            annual=ei.loc[(ANNUAL,method,idx)];assert r.probability==annual.probability and r.score==annual.score and r.direction_up==annual.direction_up;exact_ensemble_fallback+=1
    ...
    for name,source,keys in [('ensemble_metrics',ensemble,['history','method']),('seed_metrics',models,['history','method','seed']),('state_metrics',joined,['history','method','state'])]:
        ...
        for row in csv(name).to_dict('records'):
            ...
            count+=1
    return dict(independent_learned_seed_predictions=len(details),exact_zero_offset_predictions=zeros,exact_ensemble_fallback_predictions=exact_ensemble_fallback,maximum_prediction_gap=maxgap,independent_metric_cells=count)
...
def check_statistics():
    e=csv('ensemble_predictions');pairs=read(OUT/'primary_comparisons.json');assert len(pairs)==72;expected_specs=[dict(window=w['name'],**s) for w in cfg()['windows'] for s in cfg()['primary_comparisons_per_window']];assert [{k:r[k] for k in spec} for r,spec in zip(pairs,expected_specs)]==expected_specs;ps=[]
    ...
    adjusted=[0.]*72;last=0.
    for rank,i in enumerate(sorted(range(72),key=lambda i:ps[i])):last=max(last,min(1.,ps[i]*(72-rank)));adjusted[i]=last
...
def main():
    ...
    for n in ['correction_heads.json','correction_parameters.csv']:assert sha(OUT/n)==sha(PREV/'results'/n)
