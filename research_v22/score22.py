...
def main():
    check_frozen();fit=check_phase('training');assert fit['all_converged'] and fit['new_primary_fits']==60
    ...
    for h in read(OUT/'heads.json'):
        ...
        if h['source_job']:
            for i,row in enumerate(rows):components.append(dict(job=h['job'],method=h['method'],cutoff=h['cutoff'],seed=h['seed'],row_index=int(row),date=obs.date.iloc[row],gate=float(u[i]),parent_representation_signal=float(extra['signal'][i]),product=float(extra['product'][i]),
                interaction_feature=float(xx[i,-1]),refitted_additive_logit=float(x[i]@theta[:-2]+theta[-1]),interaction_logit=float(xx[i,-1]*theta[-2]),logit=float(z[i])))
            for split,values in [('training',d['standardized']),('validation',xx)]:
                ...
                summaries.append(dict(job=h['job'],method=h['method'],cutoff=h['cutoff'],seed=h['seed'],split=split,n=len(values),interaction_coefficient=float(theta[-2]),raw_product_coefficient=float(theta[-2]/d['residual_sd']),interaction_feature_mean=float(values[:,-1].mean()),
                    interaction_feature_std=float(values[:,-1].std()),refitted_additive_mean=float(aa.mean()),interaction_logit_mean=float(bb.mean()),interaction_logit_std=float(bb.std()),total_logit_mean=float((aa+bb).mean())))
    ...
    for f in cfg()['folds']:
        for split in ['training','validation']:
            ...
            for gate in GATES:
                ...
                correlations.append(dict(cutoff=f['cutoff'],split=split,gate=gate,n=len(u),correlation_volatility20=float(np.corrcoef(u,old.volatility20)[0,1]),correlation_abs_trend60=float(np.corrcoef(u,abs(old.trend60))[0,1])))
    new=pd.concat(forecasts,ignore_index=True);assert len(new)==2610;models=pd.concat([pd.read_csv(V21/'results/model_predictions.csv',float_precision='round_trip'),new],ignore_index=True)
    ...
    ensemble=pd.concat([pd.read_csv(V21/'results/ensemble_predictions.csv',float_precision='round_trip'),e],ignore_index=True);assert len(models)==11223 and len(ensemble)==6786 and ensemble.groupby('method').size().eq(261).all()
    ...
    finish(run,files,new_probability_forecasts=2610,reused_model_records=8613,model_records=11223,ensemble_records=6786,heldout_dates=261,heldout_daily_steps=1462,interaction_components=2088)
    print('Scored2610new probabilities;26methods on261dates.',flush=True)
