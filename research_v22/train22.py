...
def main():
    ...
    for job in read(OUT/'jobs.json'):
        ...
        if job['source_job']:
            source=sources[job['source_job']];x,rows=base_inputs(source,'training');np.testing.assert_array_equal(rows,tr);d=fit_interaction(x,u,np.asarray(source['coefficients'])[:25]);xx=d['standardized'];projections+=1
        ...
        for i,t in enumerate(theta):coefs.append(dict(job=h['job'],method=h['method'],cutoff=h['cutoff'],seed=h['seed'],coordinate=i,coefficient=float(t),block='intercept' if i==len(theta)-1 else 'interaction' if h['source_job'] and i==len(theta)-2 else 'additive' if h['source_job'] else 'state'))
        save(OUT/'heads.json',heads);pd.DataFrame(traces).to_csv(OUT/'solver_trace.csv',index=False);print(f"Converged {len(heads)}/60: {h['job']}; updates={h['iterations']}",flush=True)
    assert len(heads)==60 and projections==48 and total==94650 and len(coefs)==1476
    ...
    files += [OUT/'heads.json',OUT/'solver_trace.csv'];finish(run,files,new_primary_fits=60,new_projection_fits=48,new_newton_iterations=sum(h['iterations'] for h in heads),training_feature_rows=total,all_converged=True,
        validation_scoring_during_fit=False,new_hmm_fits=0,neural_forward_passes=0,neural_training_steps=0)
    print('All60heads finished before heldout scoring.',flush=True)
