...
def enriched(ensemble):
    ...
    result=ensemble
    for frame in [old,hmm,new]:result=result.merge(frame,on=['cutoff','row_index','date'],validate='many_to_one')
    return result
...
def main():
    ...
    assert len(pairs)==52 and len(tables['state_metrics'])==1196 and len(tables['reliability_bins'])==250
    finish(run,files,primary_contrasts=52,methods=26,metric_groups=1520,state_metric_rows=1196,reliability_bins=250,pooled_descriptive_only=True)
