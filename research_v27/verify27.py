...
from contract27 import memberships
import evaluate27 as evaluation
...
def main():
    legacy.initialize();start=time.time();prep=check_frozen();assert not (OUT/'verification.json').exists();dcheck=memberships();last=read(OUT/'contract_verification.json')['completed_utc'];assert prep['finished_utc']<=last
    ...
    for r in models:
        fold=next(f for f in cfg()['folds'] if f['cutoff']==r['cutoff']);tr,te=indices(obs,fold,r['history']);hs=[next(h for h in heads if h['history']==r['history'] and h['cutoff']==r['cutoff'] and h['seed']==r['seed'] and h['method']==m) for m in LEARNED];d=arrays(hs[0]);model,state=load_model(r)
        assert state['history']==r['history'] and state['full_train_n']==fold['train_n']
        assert state['scales']==scales[f"{r['history']}_{r['cutoff']}"] and state['protocol_sha256']==prep['protocol_sha256'] and state['train_n']==len(tr)
        ...
        cs=curves[curves.history.eq(r['history'])&curves.cutoff.eq(r['cutoff'])&curves.seed.eq(r['seed'])].sort_values('epoch');assert len(cs)==20 and cs.epoch.tolist()==list(range(1,21));rng=np.random.default_rng(r['seed'])
        for row in cs.itertuples():
            assert row.order_sha256==training.array_hash(schedule(len(tr),fold['train_n'],rng)) and row.presentations==fold['train_n'] and row.optimizer_steps==(fold['train_n']+127)//128
        assert rng.bit_generator.state==state['numpy_rng_state']
        for s in state['optimizer_state_dict']['state'].values():assert int(s['step'])==20*((fold['train_n']+127)//128)
        ...
        for j,(h,x) in enumerate(zip(hs,xx)):
            ...
            if j<3:
                ...
                offset=xx[1]@t19[:-1]+t19[-1];alt=prior.previous.independent_root(offset,x[:,-1],y,cfg()['offset_probe']);gap=abs(alt['gamma']-theta[-2]);assert gap<=1e-7
        ...
        td=arrays(next(t for t in testrefs if t['history']==r['history'] and t['cutoff']==r['cutoff'] and t['seed']==r['seed']));f,native=forecasts(model,te,scales[f"{r['history']}_{r['cutoff']}"]);np.testing.assert_array_equal(f,td['features']);np.testing.assert_array_equal(native,td['native']);np.testing.assert_array_equal(te,td['row_index'])
        ...
        for h,x in zip(hs,xx):
            g=pred[pred.history.eq(r['history'])&pred.method.eq(h['method'])&pred.cutoff.eq(r['cutoff'])&pred.seed.eq(r['seed'])].sort_values('row_index');np.testing.assert_array_equal(te,g.row_index);theta=np.asarray(h['coefficients']);p=expit(x@theta[:-1]+theta[-1]);gap=float(abs(p-g.probability.to_numpy()).max());maxpred=max(maxpred,gap);assert gap<1e-10
        ...
        g=pred[pred.history.eq(r['history'])&pred.method.eq('native_mse')&pred.cutoff.eq(r['cutoff'])&pred.seed.eq(r['seed'])].sort_values('row_index');np.testing.assert_allclose(g.score,native,rtol=0,atol=1e-14);np.testing.assert_array_equal(g.direction_up,(native>0).astype(int))
        g=pred[pred.history.eq(r['history'])&pred.method.eq('training_frequency')&pred.cutoff.eq(r['cutoff'])];np.testing.assert_allclose(g.probability,y.mean(),rtol=0,atol=1e-14)
        replays.append(dict(history=r['history'],cutoff=r['cutoff'],seed=r['seed'],training_rows=len(tr),testing_rows=len(te),checkpoint_unchanged=True,training_features_bitwise_equal=True,testing_features_bitwise_equal=True,native_bitwise_equal=True))
        assert training.object_hash(model.state_dict())==r['model_sha256'];del model,values;torch.cuda.empty_cache();print(f'Verified {len(replays)}/36 backbones and {len(solutions)}/144 solvers',flush=True)
    base,base_e=frozen_baselines();pd.testing.assert_frame_equal(pred[pred.history.eq('full')].reset_index(drop=True),base,check_dtype=False,atol=1e-14,rtol=0)
    ...
    weekly=csv('seed_fusion_weekly')
    for row in weekly.itertuples():
        g=pred[pred.history.eq(row.history)&pred.method.eq(row.method)&pred.date.eq(row.date)].sort_values('seed');p=g.probability.to_numpy();assert len(p)==3
        for i in range(3):assert abs(p[i]-getattr(row,f'probability_seed_{i+1}'))<1e-14
        assert row.opposes_majority==bool((p.sum()>1.5)!=(np.median(p)>.5))
        variance=((p[0]-p[1])**2+(p[0]-p[2])**2+(p[1]-p[2])**2)/9
        assert abs(variance-row.seed_probability_variance)<1e-14
    plan=csv('calibration_membership_plan')
    for history in NEW:
        for fold in cfg()['folds']:
            cutoff=fold['cutoff'];start=f'{int(cutoff[:4])-2}-01-01';g=pred[pred.history.eq(history)&pred.method.eq(LEARNED[0])&pred.seed.eq(cfg()['seeds'][0])&pred.date.ge(start)&pred.date.le(cutoff)&pred.joint_completed.le(cutoff)]
            expected=plan[plan.history.eq(history)&plan.outer_cutoff.eq(cutoff)];assert set(g.date)==set(expected.date)
    ...
    for w in cfg()['windows']+[dict(name='pooled_2021_2026',start='2021-01-01',end='2026-08-31',n=272)]:
        independent.verify_metric_table(evaluation.select(ensemble,w),csv('ensemble_metrics')[csv('ensemble_metrics').window.eq(w['name'])],['history','method'])
        independent.verify_metric_table(evaluation.select(pred[pred.seed.ne(-1)],w),csv('seed_metrics')[csv('seed_metrics').window.eq(w['name'])],['history','method','seed'])
    assert len(pred)==13056 and len(ensemble)==4896
    assert len(solutions)==144 and len(replays)==36;assert old_evidence()==prep['old_evidence'];check_frozen()
    ...
    save(OUT/'verification.json',dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],old_files_preserved=len(prep['old_evidence']),membership_checks=len(dcheck),checkpoint_replays=36,independent_head_solutions=144,maximum_feature_algebra_gap=maxfeatures,maximum_forecast_gap=maxpred,training_feature_rows=sum(r['train_n'] for r in models),testing_feature_rows=1632,independent_metrics=True,all_predefined_comparisons_recomputed=True,artifacts={n:sha(OUT/n) for n in ['independent_solutions.csv','checkpoint_replays.csv']}))
