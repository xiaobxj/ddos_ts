...
def main():
    legacy.initialize();check_frozen();check_phase('training');assert not (OUT/'scoring_manifest.json').exists();run=manifest('scoring');obs,price,_=data();scales=read(OUT/'training_scales.json');heads=read(OUT/'heads.json');preds=[csv('full_model_predictions')];files=[];feature_refs=[]
    for r in read(OUT/'models.json'):
        ...
        del model;torch.cuda.empty_cache();print(f"Scored {len(feature_refs)}/36: {history} {cutoff} seed {r['seed']}",flush=True)
    models=pd.concat(preds,ignore_index=True);ensemble=ensemble_from(models);assert len(models)==13056 and len(ensemble)==4896
    pd.testing.assert_frame_equal(ensemble[ensemble.history.eq('full')].reset_index(drop=True),csv('full_ensemble_predictions'),check_dtype=False,atol=1e-14,rtol=0)
    ...
    files += [OUT/n for n in ['model_predictions.csv','ensemble_predictions.csv','testing_features.json']];finish(run,files,weeks=272,new_model_rows=8704,reused_model_rows=4352,ensemble_rows=4896,new_neural_test_observations=1632)
