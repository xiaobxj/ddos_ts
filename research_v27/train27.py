...
def main():
    ...
    for history in NEW:
        for fold in cfg()['folds']:
            cutoff=fold['cutoff'];values,labels,tr=load_training(history,cutoff);np.testing.assert_array_equal(tr,indices(obs,fold,history)[0]);mf=market(price,obs,tr);u=gates(price,obs,tr);y=(targets['returns'][tr]>0).astype(float);n=fold['train_n']
            for seed in cfg()['seeds']:
                ...
                for epoch in range(1,21):
                    ...
                    if epoch%10==0:print(f'Training {len(models)+1}/36 {history} {cutoff} seed {seed}: nominal epoch {epoch}/20; elapsed {time.time()-started:.1f}s',flush=True)
                state=training.capture(model,optimizer,rng,arm='combined',history=history,epoch=20,cutoff=cutoff,seed=seed,train_n=len(tr),full_train_n=n,scales=scales[f'{history}_{cutoff}'],protocol_sha256=sha(ROOT/'protocol.json'))
                p=cp/f'combined_{history}_{cutoff}_{seed}_epoch20.pt';torch.save(state,p);files.append(p)
                r=dict(history=history,cutoff=cutoff,seed=seed,epoch=20,train_n=len(tr),full_train_n=n,project_file=str(p.relative_to(PROJECT)),sha256=sha(p),model_sha256=state['model_sha256'],optimizer_sha256=state['optimizer_sha256'],rng_sha256=state['rng_sha256']);models.append(r)
                ...
                print(f'Completed {len(models)}/36 backbones and {len(heads)}/144 heads; no new weekly scoring.',flush=True)
                del model,optimizer,features,d,state;torch.cuda.empty_cache()
            del values,labels;torch.cuda.empty_cache()
    assert len(models)==36 and len(heads)==144 and len(curves)==720 and sum(r['optimizer_steps'] for r in curves)==17280
    ...
    check_frozen();finish(run,files,elapsed_seconds=time.time()-started,neural_fits=36,nominal_epochs=720,optimizer_steps=17280,head_fits=144,newton_iterations=sum(h['iterations'] for h in heads),projections=72,unique_training_feature_rows=sum(r['train_n'] for r in models),training_presentations=sum(r['presentations'] for r in curves),next_year_scoring_during_fit=False,all_converged=True)
    print('Training PASS:36backbones and144heads frozen before new scoring.',flush=True)
