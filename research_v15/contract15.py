...
def main():
    ...
    replay=archival_native_output(model,values)
    model.requires_grad_(True)
    with torch.inference_mode():original,_=legacy.predict(model,values,False)
    model.requires_grad_(False)
    np.testing.assert_array_equal(replay,original.cpu().numpy())
    assert not any(p.requires_grad or p.grad is not None for p in model.parameters())
    assert object_hash(model.state_dict())==before and torch.equal(cpu,torch.get_rng_state()) and torch.equal(cuda,torch.cuda.get_rng_state())
    tests.append('original inference flag compatibility and restoration on training inputs; no gradients or tensor/RNG mutation')
