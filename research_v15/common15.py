...
PREFLIGHT=PROJECT/'research_v15_preflight'
OUT=ROOT/'results'
CACHE=ROOT/'cache'
...
def old_evidence():
    ...
    for name,digest in read(PREFLIGHT/'results/preflight_manifest.json')['files'].items():
        result[str((PREFLIGHT/name).relative_to(PROJECT))]=digest
    p=PREFLIGHT/'results/preflight_manifest.json';result[str(p.relative_to(PROJECT))]=sha(p)
    assert len(result)==2527,len(result)
...
def input_hashes():
    ...
    files += [PREFLIGHT/'results'/n for n in ['preflight_manifest.json','preflight_diagnosis.json','heads.json']]
    return {str(p.relative_to(PROJECT)):sha(p) for p in files}
...
def archival_native_output(model,values):
    # In this torch build, requires_grad flags affect floating-point inference
    # dispatch even under inference_mode. Restore original flags only to replay
    # historical outputs, then return the feature extractor to its frozen state.
    flags=[(p,p.requires_grad) for p in model.parameters()];before=object_hash(model.state_dict())
    try:
        model.requires_grad_(True)
        with prior.audit_mode(model,False),torch.inference_mode():
            native,_=legacy.predict(model,values,False)
            result=native.cpu().numpy().copy()
    finally:
        for parameter,flag in flags:parameter.requires_grad_(flag)
    assert object_hash(model.state_dict())==before and all(p.requires_grad==flag for p,flag in flags)
