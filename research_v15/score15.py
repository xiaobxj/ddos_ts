...
def main():
    ...
    for head in heads:
        ...
        original_native=archival_native_output(model,values)
        if head['source']=='direction_bce':native_score=probability(original_native.astype(float))
        else:
            scales=meta[head['cutoff']]['scales'];native_score=original_native.astype(float)*scales['returns_sd']+scales['returns_mean']
        ...
        np.savez_compressed(path_feature,features=f,row_index=te,native_output=native,archival_output=original_native);files.append(path_feature)
