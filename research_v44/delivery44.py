...
def main():
    ...
    save(path,dict(status='PASS',completed_utc=now(),protocol_sha256=sha(ROOT/'protocol.json'),files={str(p.relative_to(ROOT)):sha(p) for p in sorted(files)},old_files_preserved=5800))
