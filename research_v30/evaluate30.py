...
def compute(models,ensemble):
    ...
    for w in cfg()['windows']:
        ...
        for history in NEW:
            for method in LEARNED:
                a=e[e.history.eq(history)&e.method.eq(method)].sort_values('date').reset_index(drop=True);b=e[e.history.eq('rolling5_annual20')&e.method.eq(method)].sort_values('date').reset_index(drop=True);changed=a.direction_up.ne(b.direction_up);good=a.direction_up.eq(a.actual_up)
    ...
    return dict(ensemble_metrics=pd.DataFrame(metrics),yearly_metrics=yearly,seed_metrics=pd.DataFrame(seeds),seed_yearly_metrics=seed_yearly,reliability_bins=pd.DataFrame(bins),direction_changes=pd.DataFrame(changes),seed_fusion_weekly=weekly,seed_fusion_diagnostics=diag,next_quarter_diagnostics=next_quarter_diagnostics(csv('feedback_pool'),csv('events'))),pairs
def main():
    ...
    save(OUT/'primary_comparisons.json',pairs);files.append(OUT/'primary_comparisons.json');assert len(pairs)==40
    finish(run,files,primary_contrasts=40,histories=5,methods=6,seed_fusion_rules_changed=False,calibration_fits=0,historical_extension=True,independent_holdout=False)
