...
def main():
    ...
    new=pd.concat(parts,ignore_index=True);assert len(new)==8704;models=pd.concat([base,new],ignore_index=True);assert len(models)==95744 and not models.duplicated(['history','method','seed','date']).any();ensemble=v37.ensemble_from(models);assert len(ensemble)==35904
    ...
    check_frozen();finish(run,files,new_model_rows=8704,new_learned_seed_rows=6528,gate_changes_during_scoring=0);print('Two fixed monthly calibration policies scored on272weeks; old20histories preserved.',flush=True)
