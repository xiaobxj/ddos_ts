...
def main():
    check_frozen();check_phase('fitting');assert not (OUT/'validation_manifest.json').exists();run=manifest('validation');g,v=validate(csv('weekly_signal_bank'),read(OUT/'correction_heads.json'),csv('schedule'),csv('split_membership'));assert len(g)==1088;files=[]
    for name,t in [('gate_decisions',g),('validation_predictions',v)]:p=OUT/f'{name}.csv';t.to_csv(p,index=False);files.append(p)
    check_frozen();finish(run,files,gate_cells=len(g),accepted_cells=int(g.accepted.sum()),parameters_refit_after_validation=0,future_scoring=False);print(f'Validation complete: {g.accepted.sum()}/1088 cells accepted; all gates frozen before future scoring.',flush=True)
