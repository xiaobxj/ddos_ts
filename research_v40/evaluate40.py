...
def compute(models,ensemble):
    ...
    weekly=pd.DataFrame(weekly).drop(columns='Index');assert len(weekly)==2176
    for p in periods():
    ...
    return dict(ensemble_metrics=pd.DataFrame(metrics),seed_metrics=pd.DataFrame(sm),state_metrics=pd.DataFrame(state),weekly_policy_effects=weekly,direction_changes=pd.DataFrame(changes),month_outcomes=pd.DataFrame(outcomes),all_2026_cases=weekly[weekly.year.eq(2026)]),pairs
def main():
    ...
    p=OUT/'primary_comparisons.json';save(p,pairs);files.append(p);assert len(pairs)==60;finish(run,files,exploratory_comparisons=60,independent_holdout=False)
