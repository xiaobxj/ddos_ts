...
def main():
    ...
    save(OUT/'verification.json',dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,protocol_sha256=prep['protocol_sha256'],source_sha256=prep['source_sha256'],old_files_preserved=3672,data_contract=dcheck,checkpoint_replays=18,independent_head_solutions=72,maximum_feature_algebra_gap=maxfeatures,maximum_forecast_gap=maxpred,training_feature_rows=54252,testing_feature_rows=816,independent_metrics=True,all_predefined_comparisons_recomputed=True,artifacts={n:sha(OUT/n) for n in ['independent_solutions.csv','checkpoint_replays.csv']}))
