...
def compute():
    ...
    for w in windows():
        ...
        for (method,seed),g in select(models[models.method.isin(['native_mse','learned_probe','learned_clip','learned_market'])],w).groupby(['method','seed']):
    ...
    for w in cfg()['windows']:
        ...
        for candidate,parent in CANDIDATES.items():
            ...
            c=t.loc[candidate]
            flags=dict(accuracy_beats_parent=bool(c.accuracy>t.loc[parent,'accuracy']),accuracy_beats_native=bool(c.accuracy>t.loc['native_mse','accuracy']),
                accuracy_beats_frequency=bool(c.accuracy>t.loc['training_frequency','accuracy']),brier_beats_parent=bool(c.brier<t.loc[parent,'brier']),
                brier_beats_market4=bool(c.brier<t.loc['market4','brier']),brier_beats_frequency=bool(c.brier<t.loc['training_frequency','brier']),
            ...
            assessments.append(dict(window=w['name'],method=candidate,parent=parent,counts=counts,flags=flags,window_descriptive_pass=all(flags.values()),
        ...
        for method in NEW_METHODS:
            for reference in ([CANDIDATES[method],'native_mse','market4'] if method in CANDIDATES else ['training_frequency']):
                a=e[e.method.eq(method)].set_index('date');b=e[e.method.eq(reference)].set_index('date').loc[a.index]
                new=a.direction_up.eq(a.actual_up);old=b.direction_up.eq(b.actual_up)
                changes.append(dict(window=w['name'],method=method,reference=reference,changed=int(a.direction_up.ne(b.direction_up).sum()),
        ...
        for partition,labels in PARTITIONS.items():
            for label in labels:
                ...
                for method in METHODS:
                    g=s[s.method.eq(method)];m=metric(g);diagnostics.append(dict(window=w['name'],partition=partition,state=label,method=method,sparse=len(g)<20,
                        sample_share=len(g)/w['n'],brier_difference_vs_frequency=float(loss(g,'brier').mean()-loss(f,'brier').mean()) if len(g) and g.probability.notna().all() else None,**m))
...
def main():
    ...
    finish(run,files,primary_contrasts=14,methods=10,years=6,windows=2,pooled_descriptive_only=True,state_metric_rows=len(tables['state_metrics']))
