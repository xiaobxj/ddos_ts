...
def main():
    prep=check_frozen(False);assert not (OUT/'contract_verification.json').exists();started=now();checks=memberships_check();cases=boundary_cases();check_frozen(False);checks.to_csv(OUT/'membership_checks.csv',index=False)
