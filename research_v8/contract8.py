        ...
        for fold in cfg()['folds']:
            ...
            for policy in cfg()['policies']:
                ...
                if policy.startswith('disjoint'):
                    phase = int(policy[-1]); anchor0 = int(obs.anchor.iloc[full[0]])
                    self.assertTrue((((obs.anchor.iloc[tr].to_numpy()-anchor0)%6)==phase).all())
                    for other in ['disjoint0','disjoint1','disjoint2']:
                        if other != policy:
                            other_tr,_,_,_ = load_policy(fold['cutoff'],other)
                            self.assertEqual(len(np.intersect1d(tr,other_tr)),0)
