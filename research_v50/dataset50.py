...
def observations(frame,cutoff):
    ...
    # R5 ingested R4's observation CSV with pandas' default float parser before
    # freezing targets.npz. Preserve that numerical target vintage explicitly.
    legacy_returns=pd.read_csv(io.StringIO(pd.DataFrame({'exec_return':returns}).to_csv(index=False))).exec_return.to_numpy(float)
    return f,obs,dict(returns=legacy_returns,auxiliary=np.asarray(aux,float))
