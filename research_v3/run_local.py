...
def writable_masks(obs, cutoff, end, eligible=None):
    # pandas 3 returns read-only views; copy before applying paired exclusions.
    train, test = masks(obs, cutoff, end)
...
def main():
    ...
    for cutoff, end in periods(2018, 2020):
        tr, te = writable_masks(obs, cutoff, end)
        te &= validation
        if not te.any():
            continue
        ...
        for cutoff, end in periods(*years):
            for kind in ['ridge', 'hgb']:
                for variant in ['target', 'target_matched', 'cross']:
                    ...
                    tr, te = writable_masks(obs, cutoff, end, eligible if variant != 'target' else None)
                    te &= allowed
                    if not te.any():
                        continue
