...
def main():
    ...
    for head in heads:
        ...
        if head['source']=='direction_bce':old_score=expit(vnative.astype(float));np.testing.assert_array_equal(old.direction_up,old_score>.5)
        else:
            scales=meta[cutoff]['scales'];old_score=vnative.astype(float)*scales['returns_sd']+scales['returns_mean']
    ...
    result=dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,model_states_replayed=18,
        ...
        maximum_alternate_gradient_inf=max(r['alternate_gradient_inf'] for r in independent),previous_files_preserved=2479,
