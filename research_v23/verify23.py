...
import evaluate23 as evaluation
import verify17 as audit
import verify15 as independent
from contract23 import synthetic
...
def verify_runs(frame,gate,summary,saved):
    col='order_state';high='positive_order'
...
def main():
    ...
    fit=read(OUT/'training_manifest.json');assert fit['all_converged'] and fit['new_primary_fits']==30 and fit['new_projection_fits']==24 and not fit['validation_scoring_during_fit']
    ...
    for split in ['training','validation']:
        ...
        for f in cfg()['folds']:
            ...
            valid=np.isfinite(scalar).all(axis=1);labels=np.where(valid,np.where(scalar[:,-1]<-.05,'negative_order',np.where(scalar[:,-1]>.05,'positive_order','near_zero_order')),'missing')
    ...
    heads=read(OUT/'heads.json');jobs={j['job']:j for j in read(OUT/'jobs.json')};sources={s['job']:s for s in read(OUT/'source_heads.json')};original={s['job']:s for s in read(V19/'results/heads.json')};assert len(heads)==len(jobs)==30 and all(s==original[k] for k,s in sources.items())
    ...
    oldpred=pd.read_csv(V19/'results/model_predictions.csv',float_precision='round_trip');settings=cfg()['independent_solver'];ps=cfg()['projection_verification'];solutions=[];projections=[];forecasts=[];total=0;testtotal=0;maxerror=0.;maxparent=0.
    for h in heads:
        ...
        if h['source_job']:
            ...
            ray=source_ray(source);replay=fit_interaction(x,u,ray)
            ...
            for key,value in dict(gate=uu,parent_representation_signal=extra['signal'],product=extra['product'],interaction_feature=val[:,-1],refitted_parent_logit=xv@theta[:-2]+theta[-1],refitted_base_logit=xv[:,:-1]@theta[:-3]+theta[-1],refitted_original_interaction_logit=xv[:,-1]*theta[-3],interaction_logit=val[:,-1]*theta[-2],logit=design(val)@theta).items():np.testing.assert_allclose(comp[key],value,rtol=0,atol=1e-9)
        ...
        assert cg.block.eq('original_interaction').sum()==int(bool(h['source_job']))
    ...
    assert total==47325 and testtotal==1305 and len(coefs)==762 and len(projections)==24 and len(parts)==1044 and len(summary)==48 and len(preds)==12528 and len(ensemble)==7569
    for filename,frame in [('model_predictions.csv',preds),('ensemble_predictions.csv',ensemble)]:
        old=pd.read_csv(V22/'results'/filename,float_precision='round_trip');g=frame[frame.method.isin(old.method.unique())];keys=['method','date']+(['seed'] if 'seed' in old else []);pd.testing.assert_frame_equal(old.sort_values(keys).reset_index(drop=True),g[old.columns].sort_values(keys).reset_index(drop=True),check_dtype=False,rtol=0,atol=1e-15)
    new=pd.read_csv(OUT/'new_model_predictions.csv',float_precision='round_trip');assert len(new)==1305;pd.testing.assert_frame_equal(new.reset_index(drop=True),preds[preds.method.isin(list(CANDIDATES)+list(CONTROLS.values()))][new.columns].reset_index(drop=True),check_dtype=False,rtol=0,atol=0)
    ...
    assert groups==1868 and len(tables['reliability_bins'])==280
    ...
    for r in pd.read_csv(OUT/'state_correlations.csv',float_precision='round_trip').itertuples():
        ...
        state=states[(r.split,r.cutoff)];audit.near(float(np.corrcoef(u,state.efficiency60)[0,1]),r.correlation_efficiency60);audit.near(float(np.corrcoef(u,state.relative_volatility)[0,1]),r.correlation_relativevol)
    ...
    result=dict(status='PASS',completed_utc=now(),elapsed_seconds=time.time()-start,previous_files_preserved=3292,new_primary_fits=30,independent_solutions=30,independent_qr_projections=24,state_split_checks=12,future_prefix_checks=18,
        new_hmm_fits=0,new_neural_forward_passes=0,neural_training_steps=0,inherited_neural_replay_states=18,training_feature_rows=47325,new_probability_forecasts=1305,reused_model_records=11223,model_records=12528,ensemble_records=7569,
        metric_groups=1868,state_metric_rows=1508,reliability_bins=280,primary_contrasts=26,independent_holdout_dates=0,maximum_new_forecast_error=maxerror,maximum_parent_forecast_error=maxparent,maximum_state_formula_gap=maxstategap,
