...
def enriched(ensemble):
    old=pd.read_csv(OUT/'validation_states.csv');hmm=pd.read_csv(OUT/'hmm_signal_states.csv')[['cutoff','row_index','date','hmm_bucket']];new=pd.read_csv(OUT/'validation_signal_states.csv')[['cutoff','row_index','date']+list(STATE_PARTITIONS)]
    result=ensemble
    inherited=pd.read_csv(V22/'results/validation_signal_states.csv')[['cutoff','row_index','date']+list(previous.STATE_PARTITIONS)]
    for frame in [old,hmm,inherited,new]:result=result.merge(frame,on=['cutoff','row_index','date'],validate='many_to_one')
    return result
...
def compute():
    ...
    for w in cfg()['windows']:
        ...
        for candidate,parent in CANDIDATES.items():
            ...
            for ref in [parent,OLD_INTERACTIONS[candidate],'learned_hmm_interaction' if candidate.startswith('learned') else 'raw_hmm_interaction','native_mse','training_frequency',CONTROLS[GATE_MAP[candidate]]]:
    ...
    tables.update(state_metrics=pd.DataFrame(diagnostics),direction_changes=pd.DataFrame(changes));return tables,pairs,assess
def main():
    ...
    assert len(pairs)==26 and len(tables['state_metrics'])==1508 and len(tables['reliability_bins'])==280
    finish(run,files,primary_contrasts=26,methods=29,metric_groups=1868,state_metric_rows=1508,reliability_bins=280,pooled_descriptive_only=True)
