...
def compute():
    ...
    for method,parent in CANDIDATES.items():
        ...
        flags=dict(early_accuracy_retained=bool(t.loc[(early,method),'accuracy']>=t.loc[(early,WEAK[method]),'accuracy']),late_parent_accuracy_retained=bool(t.loc[(late,method),'accuracy']>=t.loc[(late,parent),'accuracy']),early_brier_retained=bool(t.loc[(early,method),'brier']<=t.loc[(early,WEAK[method]),'brier']),late_brier_retained=bool(t.loc[(late,method),'brier']<=t.loc[(late,WEAK[method]),'brier']))
        targets.append(dict(method=method,**flags,all_targets_met=all(flags.values()),independent_confirmation=False))
...
def main():
    ...
    assert len(pairs)==64 and len(tables['state_metrics'])==1716 and len(tables['reliability_bins'])==320
    finish(run,files,primary_contrasts=64,methods=33,metric_groups=2130,state_metric_rows=1716,reliability_bins=320,pooled_descriptive_only=True)
